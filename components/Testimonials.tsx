...
export default function Testimonials() {
  return (
    <section id="testimonials" className="section-shell py-9">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {testimonials.map(([name, role, quote]) => (
          ...
          >
            <div className="grid h-8 w-8 place-items-center rounded-full bg-gradient-to-br from-purple-500 to-blue-500 text-[16px] font-bold">
              {name[0]}
            </div>
            <p className="mt-3 text-[16px] leading-5 text-slate-300">
              “{quote}”
            </p>
            <strong className="mt-2 block text-[16px]">{name}</strong>
            <small className="text-[16px] text-slate-500">{role}</small>
            <div className="mt-2 text-[14px] text-yellow-400">★★★★★</div>
          </article>
        ))}
      </div>
    </section>
